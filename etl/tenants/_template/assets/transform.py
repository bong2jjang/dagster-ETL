"""
Template Tenant - Custom Transform Logic

이 파일을 복사하여 테넌트별 커스텀 Transform 로직을 구현하세요.

사용법:
1. 이 파일을 tenants/{tenant_id}/assets/transform.py로 복사
2. 필요한 함수만 구현 (나머지는 common 사용)
3. __init__.py의 CUSTOM_TRANSFORM_FUNCTIONS에 등록

참고:
- 함수 시그니처는 common/assets/transform.py와 동일하게 유지
...
logger = ETLLogger("template.transform")
...
def transform_aps_wip_logic(
    ...
    """
    커스텀 WIP Transform 로직 예시

    common/assets/transform.py의 transform_aps_wip_logic을 기반으로
    테넌트별 특수 로직을 추가합니다.
    ...
    logger.info(f"[{tenant_id}] Using custom WIP transform logic")
